...
function createUserSchema(){
    ...
    // 스키마 객체의 virtual() 함수를 이용해 info 속성을 지정 -> 스키마의 다른 속성과 동일하게 사용 가능
    // virtual()로 속성 지정 시 set()/get() 메소드 호출 가능
    UserSchema.virtual('info').set(function(info){
        var splitted = info.split(' ');
        this.id = splitted[0];
        this.name = splitted[1];
        console.log('virtual info 설청함 : %s, %s', this.id, this.name);
...
function doTest(){
    ...
    var user = new UserModel({'info' : 'test01 아이즈원두테스트'});
